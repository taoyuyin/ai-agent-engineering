...
每个长期保留的目录都应包含 `README.md`，至少说明：

- 目录职责和与全书的关系；
- 当前已经实现的文件与运行入口；
- 模型、Runtime 和确定性控制面的边界；
- 环境依赖、验证方式和生产限制；
- 尚未实现时明确标注“设计契约”，不能把目标结构写成现状。

空目录不使用一句话占位。若当前没有实现，README 应给出业务目标、目标 Contract、验收标准和进入实现阶段的条件。

## 12. 示例完成度标记

仓库统一使用三种状态：

- `已实现`：代码、依赖、运行入口和验证说明齐全；
- `教学 MVP`：最小代码可验证章节核心，但明确生产限制；
- `设计契约`：只有经过审查的架构和验收标准，没有声称可运行。

README、roadmap 和源码状态必须一致。
